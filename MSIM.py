...
# MCP Server Integration (corrected import)

# ----------------------------------------------------------------------

try:
import anythingllm_mcp
anything_mcp = anythingllm_mcp.mcp
TOOLS = anythingllm_mcp.TOOLS
MCP_AVAILABLE = True
except ImportError as e:
logger.error(f"Failed to import anythingllm_mcp: {e}")
logger.error("Make sure you ran 'uv pip install ./submodules/anythingllm-mcp/'")
sys.exit(1)
...
# Helper to get tool definitions
...
app = FastAPI(title="MSIM MCP Server", version="1.0.5")
app.add_middleware(
CORSMiddleware,
allow_origins=["*"],
